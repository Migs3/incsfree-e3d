...
#define HBOOT_IN_FILE "/dev/block/mmcblk0p13"
#define HBOOT_OUT_FILE "/dev/block/mmcblk0p13"
#define RECOVERY_IN_FILE "/dev/block/mmcblk0p22"
#define RECOVERY_OUT_FILE "/dev/block/mmcblk0p22"
...
int main(int argc, const char **argv)
{
    ...
    if (recovery){
    	// backup partition 22
    	printf("Backing up current partition 22 and installing specified recovery image...\n");

    	backupFile = malloc(snprintf(0, 0, "/sdcard/part22backup-%lu.bin", ourTime) + 1);
        if(!backupFile) {
    		fprintf(stderr, "Failed to allocate memory for backup file name.. lol\n");
    		return 1;
        }
        sprintf(backupFile, "/sdcard/part22backup-%lu.bin", ourTime);
...
int patchModVermagic(uint8_t *mod, int mod_size, void *modBuffer, int *modNeededBuffer){
	...
	while(current) {
		if(strstr(current->string, "vermagic=")) {
			free(current->string);
			current->string = malloc(snprintf(0, 0, "vermagic=%s SMP preempt mod_unload ARMv7 ", kernInfo.release) + 1);
			if(!current->string) {
				fprintf(stderr, "Failed to allocate memory for vermagic string... lol.\n");
				return 0;
			}
			sprintf(current->string, "vermagic=%s SMP preempt mod_unload ARMv7 ", kernInfo.release);
		}
